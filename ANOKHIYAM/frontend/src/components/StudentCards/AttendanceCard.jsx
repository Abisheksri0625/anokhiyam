...
const AttendanceCard = ({ overall = 92, thisMonth = 96, target = 85 }) => {
  return (
    <div className={`${styles.card} ${styles.attendanceCard}`}>
      ...
      <div className={styles.cardContent}>
        <div className={styles.mainValue}>{overall}%</div>
        <div className={styles.changeValue}>Overall Attendance</div>
        <div className={styles.description}>
          {thisMonth}% this month • Target: {target}%
        </div>
      </div>
      
      <div className={styles.attendanceStatus}>
        <span className={overall >= target ? styles.statusGood : styles.statusWarning}>
          {overall >= target ? '✓ Above target' : '⚠ Below target'}
        </span>
      </div>
    </div>
  );
